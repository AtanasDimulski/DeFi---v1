...
contract Exchange is ERC20 {
    ...
    address public tokenTAddress;

    // Exchange is inheriting ERC20, becase our exchange would keep track of Crypto Dev LP tokens
    constructor(address _tokenTAddress) ERC20("TokenT LP Token", "TLP") {
        require(_tokenTAddress != address(0), "Token address passed is a null address");
        tokenTAddress = _tokenTAddress;
    }

    //Returns the amount of `TokenT` held by the contract
    function getReserve() public view returns (uint) {
        return ERC20(tokenTAddress).balanceOf(address(this));
    }
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        uint tokenTReserve = getReserve();
        ERC20 tokenT = ERC20(tokenTAddress);

    if(tokenTReserve == 0) {
        tokenT.transferFrom(msg.sender, address(this), _amount);
        ...
        uint tokenTAmount = (msg.value * tokenTReserve)/(ethReserve);
        require(_amount >= tokenTAmount, "Amount of tokens sent is less than the minimum tokens required");
        // transfer only (cryptoDevTokenAmount user can add) amount of `Crypto Dev tokens` from users account
        // to the contract
        tokenT.transferFrom(msg.sender, address(this), tokenTAmount);
    ...
    */
    function removeLiquidity(uint _amount) public returns (uint , uint) {
        ...
        uint dealndrTokenAmount = (getReserve() * _amount)/ _totalSupply;
        ...
        ERC20(delandrTokenAddress).transfer(msg.sender, dealndrTokenAmount);
        return (ethAmount, dealndrTokenAmount);
    }
